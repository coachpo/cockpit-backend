...
## WHERE TO LOOK
- `main.go`: version metadata, flag definitions, `.env` load, `DEPLOY` cloud standby, `NACOS_ADDR` bootstrap, and runtime mode branching.
...
- Build metadata is injected through linker flags into `Version`, `Commit`, and `BuildDate`.
