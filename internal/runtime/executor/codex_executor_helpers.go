...
import (
	...
	codexauth "github.com/coachpo/cockpit-backend/internal/auth/codex"
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, req cliproxyexecutor.Request, rawJSON []byte) (*http.Request, error) {
	...
	if from == "openai-response" {
		...
		if apiKey := strings.TrimSpace(apiKeyFromContext(ctx)); apiKey != "" {
			cache.ID = uuid.NewSHA1(uuid.NameSpaceOID, []byte("cockpit:codex:prompt-cache:"+apiKey)).String()
		}
	}
...
func newCodexStatusErr(statusCode int, body []byte, headers http.Header) statusErr {
	err := newHTTPStatusErr(statusCode, body, headers)
