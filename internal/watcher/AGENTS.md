...
## OVERVIEW
Config and auth hot-reload pipeline. Combines Nacos-backed config/auth watching, auth synthesis, redacted diffs, and batched update dispatch.
...
- `synthesizer/`: config-backed auth synthesis.
- `diff/`: redacted change reporting and hashing.

## LOCAL CONVENTIONS
- Config changes and Nacos-backed auth changes flow through the same update queue.
