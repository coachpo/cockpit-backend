...
func TestBuildBackendCallbackURL_UsesLocalhostAuthCallbackForLoopbackOrigins(t *testing.T) {
	tests := []struct {
		name   string
		origin string
		want   string
		...
		{
			name:   "non loopback origin keeps host",
			origin: "https://cockpit.example.com:9443",
			want:   "https://cockpit.example.com:9443/auth/callback",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("failed to parse origin %q: %v", tt.origin, err)
			}
			got, err := buildBackendCallbackURL(originURL)
			if err != nil {
...
func TestCreateOAuthSession_UsesBackendLoopbackRedirect(t *testing.T) {
	...
	if got := parsed.Query().Get("redirect_uri"); got != codex.RedirectURI {
		t.Fatalf("expected redirect_uri to use fixed Codex loopback callback, got %q", got)
	}
	if forwarderTarget != "http://localhost:8317/auth/callback" {
		t.Fatalf("expected loopback forwarder to target backend callback route, got %q", forwarderTarget)
	}
	session, err := LoadOAuthSession(body.State)
	if err != nil {
		t.Fatalf("expected stored oauth session, got %v", err)
	}
	if session.Status != oauthStatusPending || session.RedirectURI != codex.RedirectURI || session.Provider != "codex" {
...
func TestGetOAuthCallback_UsesStoredRedirectURIForExchangeAndReturnsHTML(t *testing.T) {
	gin.SetMode(gin.TestMode)
	oldStore := oauthSessions
	oauthSessions = newOAuthSessionStore(oauthSessionTTL)
	defer func() { oauthSessions = oldStore }()
	...
	if err := RegisterOAuthSessionWithRedirect("state-456", "codex", codex.RedirectURI, &codex.PKCECodes{CodeVerifier: "verifier", CodeChallenge: "challenge"}); err != nil {
		t.Fatalf("failed to register oauth session: %v", err)
	}
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/auth/callback?state=state-456&code=auth-code", nil)

	h.GetOAuthCallback(ctx)
	...
	session, err := LoadOAuthSession("state-456")
	if err != nil {
...
func TestGetOAuthCallback_SetsSessionErrorAndReturnsHTML(t *testing.T) {
	gin.SetMode(gin.TestMode)
	oldStore := oauthSessions
	oauthSessions = newOAuthSessionStore(oauthSessionTTL)
	defer func() { oauthSessions = oldStore }()

	if err := RegisterOAuthSessionWithRedirect("state-error", "codex", codex.RedirectURI, &codex.PKCECodes{CodeVerifier: "verifier", CodeChallenge: "challenge"}); err != nil {
		t.Fatalf("failed to register oauth session: %v", err)
	}
	...
	if !strings.Contains(rec.Body.String(), "Authentication failed") || !strings.Contains(rec.Body.String(), "access_denied") {
		t.Fatalf("expected error html response, got %s", rec.Body.String())
	}
	session, err := LoadOAuthSession("state-error")
	if err != nil {
		t.Fatalf("expected oauth session to remain queryable, got %v", err)
	}
	if session.Status != oauthStatusError || session.Error != "access_denied" {
		t.Fatalf("expected oauth session error state after backend callback error, got %+v", session)
	}
}
