...
## OVERVIEW
Gin-based HTTP layer. Owns server construction, middleware ordering, `/v1*` routes, `/api`, OpenAPI surface tests, websocket attachment, and the `handlers/` subtree for management endpoints.

## WHERE TO LOOK
- `server.go`: `Server`, `NewServer`, shared route setup, hot-reload handling.
- `server_management.go`: lazy management surface wiring and retained-route mounting for `/api`.
...
## LOCAL CONVENTIONS
- Management routes on the trimmed `/api` surface are mounted directly today; if auth enforcement changes, update the route wiring, tests, and docs together.
