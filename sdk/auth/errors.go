...
func (e *ProjectSelectionError) Error() string {
	if e == nil {
		return "cockpit auth: project selection required"
	}
	return fmt.Sprintf("cockpit auth: project selection required for %s", e.Email)
}
...
func (e *EmailRequiredError) Error() string {
	if e == nil || e.Prompt == "" {
		return "cockpit auth: email is required"
	}
	return e.Prompt
}
