...
if [[ $# -gt 0 ]]; then
  case "$1" in
    --with-usage)
      echo "Error: --with-usage is no longer supported because the management usage endpoints were removed."
      ;;
    *)
...
case "$choice" in
  ...
  2)
    ...
    export COCKPIT_IMAGE="cockpit:local"
    export CLI_PROXY_IMAGE="${COCKPIT_IMAGE}"
